...
- **Agents tab** — all agents with status, provider/model (editable via modal with schedule run mode selector), sessions, messages, tokens, cost, errors. File viewer buttons on each agent to view daily logs, SOUL.md, and session snapshots.
...
- **Pre-compaction flush** — when the session window fills up and old messages are trimmed, the agent summarizes the expiring context and stores it in both vector memory and the daily log markdown.
- **Mid-run memory persist** — when the Context Window Guard compacts tool rounds mid-run, the summary is stored in vector memory and appended to the daily log so it survives later session trimming.
- **Daily logs** — after each run, a structured entry (task, tools used, result excerpt, token count) is appended to `memory/YYYY-MM-DD.md`. No LLM call. Today's log and yesterday's log are auto-injected into the system prompt (combined 6000 char budget) for cross-midnight continuity.
...
**Structured execution** (`schedule_run_mode: "structured"`) bypasses Anthropic's tool_use protocol entirely for scheduled runs. The agent makes exactly 2 LLM calls — one to plan data gathering, one to plan actions — both outputting JSON that Python executes directly. This eliminates tool schema overhead (~16K tokens per round) and reduces a typical scheduled run from ~150K tokens to ~15K tokens. Interactive chat still uses the standard tool_use loop. **Batch mode** (`"batch"`) wraps structured execution in the xAI Batch API for an additional 50% cost discount, with automatic fallback to real-time on timeout.
...
| `schedule_run_mode` | `"tool_use"` | `"tool_use"` (standard loop), `"structured"` (2-phase JSON, no tool schemas), or `"batch"` (structured + xAI Batch API at 50% cost) |
...
PATCH /v1/agents/{name}/config  Update agent provider/model/run mode
GET  /v1/agents/{name}/files    List viewable workspace files
GET  /v1/agents/{name}/files/{path}  Read a workspace file (SOUL.md, daily logs, snapshots)
